...
def main():
	os.system('clear')
	print(bn)
	print(f"{flag}     Ŧ๏гɭคק ๔ย๓קєг ๒ץ к๏รђץ๏ภﻮ     {off}")
	print()
	...
	if _0 == '3':
		...
		for _ in range(int(_3)):
			print(f"{cyan}>> {yellow}𝚂𝙰𝙱𝙰𝚁 𝚂𝙰𝚈𝙰𝙽𝙶. . . . . ke {white}: {_+1}")
			collect(f'{_1}/{_*20}')
		...
		print(f'\n{green}[{white}✓{green}]{white}Total halaman : {green}{_3}{off}\n')
		for _ in range(int(_3)):
			print(f"{cyan}>> {yellow}SABAR COK. . . . . ke {white}: {_+1}")
			collect(f'{_1}/{_*20}')
		for _ in range(len(yxc)):
			__ = json.loads(json.dumps(yxc[_]))['nim']
			with open(_2,'a') as o_:
